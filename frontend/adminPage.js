    ...
    window.addRoomField = function() {
      ...
      roomDiv.className = 'border border-gray-300 rounded-lg p-4 space-y-3 bg-gray-50';
      roomDiv.id = roomId;
      roomDiv.innerHTML = `
    ...
    const defaultMeals = [
      {
        ...
        color: "#E4D00A",
        bgColor: "#F9F6C1",
        features: ["Breakfast buffet", "Lunch (3-course menu)", "Dinner (4-course menu)", "Complimentary tea & coffee"]
      },
      {
        id: "half-board",
        name: "Half Board",
        ...
        color: "#C0C0C0",
        bgColor: "#F0F0F0",
        features: ["Breakfast buffet", "Dinner (3-course menu)", "Complimentary tea & coffee"]
      },
      {
        id: "breakfast-only",
        name: "Breakfast Only",
        ...
        color: "#CD7F32",
        bgColor: "#FFE2C4",
        features: ["Full breakfast buffet", "Fresh juices & beverages", "Continental & Egyptian options"]
      },
      {
        id: "room-only",
        name: "Room Only",
    ...
    function displayHotels(hotels) {
      ...
      tbody.innerHTML = hotels.map(hotel => {
        ...
        const imageUrl = hotel.image || 'https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=64&h=64&fit=crop';
        return `
        <tr class="hover:bg-gray-50">
          <td class="px-6 py-4">
            <div class="relative">
              <img src="${imageUrl}" alt="${hotel.name}" class="w-16 h-16 object-cover rounded-lg" onerror="this.src='https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=64&h=64&fit=crop'" title="${imageUrl}" />
          ...
          <td class="px-6 py-4">
            <div class="flex space-x-2">
              <button onclick="editHotel('${hotel.id}')" class="text-blue-600 hover:text-blue-800">
                <i class="fas fa-edit"></i>
              </button>
              <button onclick="deleteHotel('${hotel.id}', '${hotel.name.replace(/'/g, "\\\'")}')" class="text-red-600 hover:text-red-800">
                <i class="fas fa-trash"></i>
              </button>
            </div>
          </td>
    ...
    window.editHotel = async function(hotelId) {
      try {
        ...
        document.querySelectorAll('.amenity-check').forEach(cb => {
          cb.checked = hotel.amenities && hotel.amenities.some(a => a.name === cb.dataset.name);
        });
    ...
    function showToast(message, type = 'success') {
      ...
      if (type === 'success') {
        toast.className = 'fixed top-20 right-4 bg-white shadow-lg rounded-lg p-4 z-50 border-l-4 border-green-500';
        icon.className = 'fas fa-check-circle text-2xl mr-3 text-green-500';
      } else {
        toast.className = 'fixed top-20 right-4 bg-white shadow-lg rounded-lg p-4 z-50 border-l-4 border-red-500';
